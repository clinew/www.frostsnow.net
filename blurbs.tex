...
\subsection{2025-02-13}
\begin{verbatim}
	>wget "https://i.imgur.com/46JvXMY.mp4"
	>HTTP request sent, awaiting response... 429 Unknown Error
	>wget --user-agent="Fcuk you" "https://i.imgur.com/46JvXMY.mp4"
	>HTTP request sent, awaiting response... 200 OK
\end{verbatim}
Why is Imgur such a user-hostile site?

\subsection{2025-01-01}
\begin{verbatim}
	Dec 30 02:12:41 hesse sshd-session[13507]: Invalid user haruhi from 151.217.62.67 port 58938
	Dec 30 02:12:41 hesse sshd-session[13507]: Received disconnect from 151.217.62.67 port 58938:11: Merry Christmas from #38c3 [preauth]
\end{verbatim}
Merry Christmas (and a Happy New Year) to you, too, \htmladdnormallink{38c3}{https://events.ccc.de/congress/2024/infos/index.html}!

\subsection{2024-09-15}
ClownFlare's latest thing is to block my access to sites such as banking, Powell's books, and others in order to verify I'm a "human" and "to review the security of my connection".
So, which is it: to verify I'm a human or to review connection security?
Actually, the latter is a blatant lie; the connection is secured over \htmladdnormallink{Transport Layer Security (TLS)}{https://en.wikipedia.org//wiki/Transport_Layer_Security}.
It gets worse, though.
I'm on a slow computer, and when I click "Verify you are human" the page runs for a bit and then \emph{force refreshes} before completing the verification.
In fact, sometimes it doesn't even get to that step before force refreshing.
Either way, despite being a legitimate user, I'm effectively blocked from my accounts without any justification or workaround provided by ClownFlare.
This isn't security.
It's just bloated shitware.

\subsection{2024-04-29 #2}
Had an issue upgrading my \htmladdnormallink{BeagleBone Black}{https://beagleboard.org/black/} kernel from the 5.10 to the 6.1 series where the ethernet interface disappeared.
Turned out the driver had been changed from \texttt{cpsw} to \texttt{cpsw-switch};
I had to enable first \texttt{NET_SWITCHDEV} and then \texttt{TI_CPSW_SWITCHDEV} in order to fix the issue.
Thanks to \texttt{zmatt} for helping debug the issue!

\subsection{2024-04-29}
Two days after the \htmladdnormallink{xz backdoor announcement}{https://www.openwall.com/lists/oss-security/2024/03/29/4}, I found the following suspect entries in my log:
\begin{verbatim}
...
\end{verbatim}
\ldots plus three similar sets of entries, and then never again.  The conclusion is obvious: I was the target.  I'm truly flattered.
